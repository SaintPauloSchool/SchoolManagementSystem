...
<script>
import { Plus, Delete, ChatDotRound, Check, Checked, List, Bell, ArrowLeft } from '@element-plus/icons-vue'
    ...
    handleSave() {
      this.$refs.formRef.validate((valid) => {
       if (valid) {
         ...
         if (options.length < 2) {
            this.$message.warning('多選題至少需要 2 個選項')
            return
          }
...
<style scoped>
/* 使用与 SingleChoiceEditor 相同的样式，只需修改卡片颜色 */
...
.title-input :deep(.el-input__wrapper.is-focus) {
  border-color: #059669;
  background: #ffffff;
  box-shadow: 0 0 0 5px rgba(16, 185, 129, 0.1), 0 6px 20px rgba(16, 185, 129, 0.2);
}
