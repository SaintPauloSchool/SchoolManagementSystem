...
public class DepartmentController extends BaseController {
     ...
     * 獲取班級樹形結構
     */
    @Log(title = "查詢班級樹", businessType = BusinessType.SELECT)
    @GetMapping("/tree")
    public AjaxResult tree() {
        String openUserId = getOpenUserId();
        List<SysDepartmentVO> tree = departmentService.getClassTreeByAdmin(openUserId);
     ...
     */
    @Log(title = "查詢班級樹（含家長）", businessType = BusinessType.SELECT)
