package com.sms.framework.interceptor;
...
import com.sms.common.json.JSON;
...
public class ApiSignatureInterceptor implements HandlerInterceptor {
    ...
    @Value("${sms.appSecret:kKhPdvEBDjck1RK7}")
    private String appSecret;

    @Value("${sms.apiTimeRange:300000}")
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        try {
            ...
            if (Math.abs(now - reqTime) > apiTimeRange) {
                ...
                return writeError(response, "x-timestamp is incorrect or expired");
            }
